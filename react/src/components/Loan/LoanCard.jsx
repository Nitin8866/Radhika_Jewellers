...
const LoanCard = ({ loan, type, onView, onPrincipalPayment, onInterestPayment }) => {
  ...
  const getDaysUntilDue = () => {
    // Try to get dueDate from multiple possible locations
    const dueDate = loan.dueDate || (loan.loans && loan.loans[0] && loan.loans[0].dueDate) || (loan.loans && loan.loans[0] && loan.loans[0].nextInterestDueDate);
    
    // Handle missing or invalid due date
    if (!dueDate || isNaN(new Date(dueDate).getTime())) {
      return null; // Return null to indicate invalid or missing due date
    }
