// react/src/components/Loan/LoanDetailModal.jsx
...
const LoanDetailModal = ({ isOpen, loanData, loanType, onClose, onPrincipalPayment, onInterestPayment }) => {
  ...
  const calculateAccruedInterest = (loan) => {
    const outstandingPrincipal = (loan.outstandingPrincipal / 100) || loan.originalAmount || 0;
    const interestRate = loan.interestRateMonthlyPct || loan.interestRate || 0;
    const dailyRate = interestRate / 30 / 100;

    let lastInterestDate = loan.takenDate;
    if (loan.paymentHistory && loan.paymentHistory.length > 0) {
      const interestPayments = loan.paymentHistory.filter(p => p.interestAmount > 0);
      if (interestPayments.length > 0) {
        interestPayments.sort((a, b) => new Date(b.date) - new Date(a.date));
        lastInterestDate = interestPayments[0].date;
      }
    }

    const now = new Date();
    const diffTime = now - new Date(lastInterestDate);
    const days = Math.max(0, Math.floor(diffTime / (1000 * 60 * 60 * 24)));

    // Use compound interest calculation to match backend
    return outstandingPrincipal * (Math.pow(1 + dailyRate, days) - 1);
  };

  const getDailyInterest = (loan) => {
    const outstandingPrincipal = (loan.outstandingPrincipal / 100) || loan.originalAmount || 0;
    const interestRate = loan.interestRateMonthlyPct || loan.interestRate || 0;
    const dailyRate = interestRate / 30 / 100;
  ...
  const handlePrincipalPayment = () => {
    onPrincipalPayment(loanData);
  };

  const handleInterestPayment = () => {
    onInterestPayment(loanData);
  };
  ...
  const outstandingPrincipal = (loanData.outstandingPrincipal / 100) || 0;
  const accruedInterest = loanData.accruedInterest || calculateAccruedInterest(loanData);
      ...
      <div className="bg-white rounded-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-xl">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-4 text-slate-500 mt-1">
                ...
                <div className="flex items-center gap-2">
                  <FileText size={16} />
                  <span className="font-medium">1 active loan</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
              <div className="flex items-center gap-4 mb-3">
                ...
                <div>
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Daily Interest</p>
                  <p className="text-2xl font-semibold text-amber-700">
                    {formatCurrency(getDailyInterest(loanData))}
                  </p>
                </div>
              </div>
              <p className="text-sm text-slate-500">
                {(loanData.interestRateMonthlyPct / 30).toFixed(2)}% per day
              </p>
            </div>
              ...
              <div className="flex items-center gap-4 mb-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-semibold text-green-600">
                    {formatCurrency((loanData.totalPrincipalPaid / 100) || 0)}
                  </p>
                </div>
              </div>
              <p className="text-sm text-slate-500">
        ...
        <div className="p-8">
          <div className="bg-slate-50 rounded-xl p-6 mb-8 border border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <p className="text-lg font-semibold text-slate-800">
                  {formatCurrency(loanData.originalAmount || (loanData.principalPaise / 100) || 0)}
                </p>
              </div>
              <div>
                <p className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-2">Loan Date</p>
                <p className="text-lg font-medium text-slate-700">
                  {formatDate(loanData.takenDate)}
                </p>
              </div>
              <div>
                <p className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-2">Status</p>
                <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${
                  loanData.status === 'ACTIVE' ? 'bg-green-100 text-green-800' :
                  loanData.status === 'PARTIALLY_PAID' ? 'bg-amber-100 text-amber-800' :
              ...
              {loanData.note && (
                <div className="md:col-span-3">
                  ...
                  <p className="text-slate-700">{loanData.note}</p>
                </div>
              )}
              {loanData.dueDate && (
                <div>
            ...
            <div className="mt-6">
              <div className="flex justify-between text-sm text-slate-600 mb-2">
                ...
                <span className="font-medium">{loanData.completionPercentage || 0}%</span>
              </div>
              <div className="w-full bg-slate-200 rounded-full h-3">
                <div 
                  className="bg-gradient-to-r from-blue-500 to-blue-600 h-3 rounded-full transition-all duration-500"
                  style={{ width: `${Math.min(loanData.completionPercentage || 0, 100)}%` }}
                ></div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            {loanData.paymentHistory && loanData.paymentHistory.length > 0 ? (
