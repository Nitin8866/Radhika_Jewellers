...
export const SilverLoanCard = ({ loan, onEdit, onView, onPayment, onSendReminder }) => {
  ...
  const formatCurrency = (amount, maxDigits = 0) => `₹${(amount || 0).toLocaleString('en-IN', { maximumFractionDigits: maxDigits })}`;
  ...
  const outstandingPrincipal = loan.outstandingAmount || 0;
  const interestRate = loan.interestRateMonthlyPct || 0;
  const monthsElapsed = loan.startDate ? 
    (new Date() - new Date(loan.startDate)) / (1000 * 60 * 60 * 24 * 30) : 0;
  const accruedInterest = outstandingPrincipal * (interestRate / 100) * monthsElapsed;
  const outstandingAmount = outstandingPrincipal + accruedInterest;
...
const SilverLoanDetailModal = ({ loan, isOpen, onClose, onEdit, onPayment, onSendReminder }) => {
  ...
  const totalWeight = loan.items?.reduce((sum, item) => sum + (item.weightGram || 0), 0) || 0;
  const loanAmount = loan.totalLoanAmount || 0;
  const outstandingPrincipal = loan.outstandingAmount || loan.currentPrincipal || loan.totalLoanAmount || 0;
  const interestRate = loan.interestRateMonthlyPct || 0;
  const monthsElapsed = loan.startDate ? 
    (new Date() - new Date(loan.startDate)) / (1000 * 60 * 60 * 24 * 30) : 0;
  const accruedInterest = outstandingPrincipal * (interestRate / 100) * monthsElapsed;
                              ...
                              <div className="space-y-1">
                                <div className="flex justify-between items-center">
                                  ...
                                  <span className="text-sm font-medium">
                                    {formatCurrency(payment.currentOutstandingAtPayment, 2)}
                                  </span>
                                </div>
                                <div className="flex justify-between items-center">
                                  ...
                                  <span className={`text-sm font-medium ${
                                    ...
                                    {payment.isFullRepayment ? 'LOAN CLOSED' : formatCurrency(payment.currentOutstandingAfterPayment, 2)}
                                  </span>
                                </div>
