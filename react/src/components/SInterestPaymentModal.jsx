...
const SInterestPaymentModal = ({ isOpen, loan, onClose, onPaymentSuccess }) => {
  ...
  useEffect(() => {
    console.log('InterestPaymentModal opened with loan:', loan); // Debug log
    if (isOpen) {
      setInterestAmount('');
      setPaymentNote('');
  ...
  const handleInterestPayment = async (e) => {
    ...
    if (!loan?._id) {
      setError('Loan ID is missing. Please select a valid loan.');
      return;
    }

    if (!interestAmount || parseFloat(interestAmount) <= 0) {
      setError('Please enter a valid interest amount greater than zero.');
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        onPaymentSuccess?.({
          ...
          reference: interestData.referenceNumber,
      ...
      console.error('Interest payment error:', error); // Debug log
      setError(error.message || 'Failed to process interest payment. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl p-6 max-w-md w-full">
          ...
          <p className="text-red-600 mb-4">Loan data not available. Please try selecting the loan again.</p>
          <button 
            onClick={onClose} 
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-200 bg-slate-50">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center shadow-md bg-gradient-to-br from-purple-500 to-purple-600">
              <span className="text-white text-sm font-bold">{getInitials(customer.name || 'Unknown')}</span>
            </div>
            <div>
              <h2 className="text-xl font-bold text-slate-900">
                Receive Interest Payment
              </h2>
              <p className="text-sm text-slate-600">{customer.name || 'Unknown Customer'}</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="bg-slate-50 p-4 rounded-xl">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-bold text-slate-900">{loan.interestRateMonthlyPct || 'N/A'}% monthly</p>
              </div>
              <div>
...
export default SInterestPaymentModal;