...
import CreateCustomerForm from './Customer/CreateCustomerForm'; // Import the CreateCustomerForm

const AddSilverLoanModal = ({ isOpen, onClose, onSave, selectedCustomer: propCustomer }) => {
  ...
  const [selectedCustomer, setSelectedCustomer] = useState(propCustomer || null); // NEW: Pre-set from prop
  ...
  const handleReset = () => {
    ...
    setSelectedCustomer(propCustomer || null);
    setSearchTerm('');
    setErrors({});
        ...
        <div className="p-6">
          <div className="space-y-6">
            ...
            <div>
              ...
              {propCustomer ? ( // NEW: If propCustomer provided, show locked customer info
                <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <p className="font-medium text-gray-900">{propCustomer.name}</p>
                  <p className="text-sm text-gray-600">{propCustomer.phone}</p>
                  {propCustomer.email && (
                    <p className="text-sm text-gray-600">{propCustomer.email}</p>
                  )}
                </div>
            ...
            {/* Silver Items - Only show if a customer is selected or create form is not visible */}
            {!showCreateCustomerForm && (
              <div>
