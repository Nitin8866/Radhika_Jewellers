...
const SItemRepaymentModal = ({ isOpen, loan, onClose, onRepaymentSuccess }) => {
  ...
  const fetchActiveItems = async () => {
    setLoading(true);
    try {
      const response = await ApiService.getActiveItemsForReturnS(loan._id);
      if (response.success) {
  ...
  const handleRepayment = async (e) => {
    ...
    try {
      ...
      const response = await ApiService.processItemReturnS(loan._id, repaymentData);
                ...
                <div key={item._id} className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h4 className="font-medium text-gray-900">{item.name || 'Silver Item'}</h4>
...
export default SItemRepaymentModal;