...
export const receiveUdharPayment = async (req, res) => {
  try {
    ...
    const previousOutstanding = udhar.outstandingPrincipal;
    udhar.outstandingPrincipal = Math.max(0, udhar.outstandingPrincipal - principalPaise);
    let overpayment = 0;
    if (udhar.outstandingPrincipal < 0) {
      overpayment = -udhar.outstandingPrincipal;
      udhar.outstandingPrincipal = 0;
    }
    ...
    // Handle overpayment by creating a new TAKEN udhar
    let newUdharId = null;
    if (overpayment > 0) {
      const newUdhar = new Udhar({
        customer: udhar.customer._id,
        udharType: 'TAKEN',
        principalPaise: overpayment,
        direction: 1,
        sourceType: 'UDHAR',
        note: `Overpayment from udhar ${udharId} - ${note || ''}`,
        outstandingPrincipal: overpayment,
        totalInstallments: 1,
        dueDate: null,
        takenDate: new Date(),
        paymentHistory: [],
      ...
      const savedNewUdhar = await newUdhar.save();
      newUdharId = savedNewUdhar._id;
      // Create transaction for new udhar
      const newTransaction = new Transaction({
        type: 'UDHAR_TAKEN',
        customer: udhar.customer._id,
        amount: overpayment / 100,
        direction: 1,
        description: `Overpayment received as new taken udhar from ${customerName}`,
        relatedDoc: savedNewUdhar._id,
        relatedModel: 'Udhar',
        category: 'INCOME',
        date: transactionDate,
        metadata: {
          paymentType: 'DISBURSEMENT',
          paymentMethod,
          originalUdharAmount: overpayment / 100,
          totalInstallments: 1,
          fromOverpayment: udharId
        }
      });
    ...
    const transaction = new Transaction({
      ...
      customer: udhar.customer._id,
      amount: (principalPaise - overpayment) / 100,
      direction: 1, // incoming
    ...
    const savedTransaction = await transaction.save();
    const totalPaidPrincipal = udhar.paymentHistory.reduce((sum, p) => sum + p.principalAmount, 0);
...
export const makeUdharPayment = async (req, res) => {
  try {
    ...
    udhar.isActive = !isFullyPaid;
    const updatedUdhar = await udhar.save();
    // Handle overpayment by creating a new GIVEN udhar
    let newUdharId = null;
    if (overpayment > 0) {
      const newUdhar = new Udhar({
        customer: udhar.customer._id,
      ...
      const newTransaction = new Transaction({
        ...
        amount: overpayment / 100,
        direction: -1,
        description: `Overpayment made as new given udhar to ${customerName}`,
        relatedDoc: savedNewUdhar._id,
        relatedModel: 'Udhar',
        category: 'EXPENSE',
        date: transactionDate,
...
export const getCustomerUdharSummary = async (req, res) => {
  try {
    ...
    let totalPaidToCollect = 0;
    let totalPaidToPay = 0;
    const givenUdhars = [];
    const takenUdhars = [];
    udhars.forEach(udhar => {
      if (udhar.udharType === 'GIVEN') {
        ...
        totalPaidToCollect += udhar.paymentHistory.reduce((sum, p) => sum + p.principalAmount, 0);
        ...
        totalPaidToPay += udhar.paymentHistory.reduce((sum, p) => sum + p.principalAmount, 0);
    ...
    const summary = {
      ...
      totalPaidToCollect: totalPaidToCollect / 100,
      totalPaidToPay: totalPaidToPay / 100,
      udhars: {
        given: givenUdhars,
        taken: takenUdhars,
        all: udhars
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          originalAmount: originalUdhar.principalPaise / 100,
          totalPrincipalPaid: originalUdhar.paymentHistory.reduce((sum, p) => sum + p.principalAmount, 0) / 100,
          outstandingBalance: originalUdhar.outstandingPrincipal / 100,
